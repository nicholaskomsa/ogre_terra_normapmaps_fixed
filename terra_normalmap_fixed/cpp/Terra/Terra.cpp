...
namespace Ogre
{
    ...
    void Terra::createHeightmap(Image2& image, const String& imageName)
    {
        ...
        auto getMinMaxHeight = [&]() {

            float min = std::numeric_limits<float>::max(), max = std::numeric_limits<float>::min();

            for (auto& h : m_heightMap) {
                min = std::min(h, min);
                max = std::max(h, max);
            }
    ...
    void Terra::createNormalTexture(void)
    {
        ...
        auto createMapTexture = [&](TextureGpu*& tex, const Ogre::String& name, const Ogre::String& materialName, TextureGpu*& texToMap) -> Pass* {


            TextureGpuManager* textureManager =
                mManager->getDestinationRenderSystem()->getTextureGpuManager();

            tex = textureManager->createTexture(
                name,
                GpuPageOutStrategy::SaveToSystemRam,
                TextureFlags::RenderToTexture | TextureFlags::AllowAutomipmaps,
                TextureTypes::Type2D);

            tex->setResolution(texToMap->getWidth(), texToMap->getHeight());

            tex->setNumMipmaps(
                PixelFormatGpuUtils::getMaxMipmapCount(tex->getWidth(),
                    tex->getHeight()));
            
            if (textureManager->checkSupport(
                PFG_R10G10B10A2_UNORM, TextureFlags::RenderToTexture | TextureFlags::AllowAutomipmaps))
            {
                tex->setPixelFormat(PFG_R10G10B10A2_UNORM);

            }else {
                tex->setPixelFormat(PFG_RGBA8_UNORM);
            }
        ...
        auto createNormalMap = [&]() {
            auto pass = createMapTexture(m_normalMapTex, "NormalMapTex_" + StringConverter::toString(getId()), "Terra/GpuNormalMapper", m_heightMapTex);
        ...
        auto createTangentMap = [&]() {
            auto pass = createMapTexture(m_tangentMapTex, "TangentMapTex_" + StringConverter::toString(getId()), "Terra/GpuTangentMapper", m_heightMapTex);
        };

        createNormalMap();
        createTangentMap();

        CompositorChannelVec finalTargetChannels(2, CompositorChannel());
        finalTargetChannels[0] = m_normalMapTex;
        finalTargetChannels[1] = m_tangentMapTex;
